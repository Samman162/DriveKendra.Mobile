...
bookingsRoute.get('/', async (c) => {
  ...
  try {
    ...
    console.warn('[Bookings] Database unavailable; returning empty bookings list:', error?.message || error);
...
bookingsRoute.post('/', async (c) => {
  ...
  try {
    ...
    if (error instanceof HttpError) throw error;
    console.error('[Bookings] Submit booking database error:', error?.message || error);
    throw new HttpError(
      503,
