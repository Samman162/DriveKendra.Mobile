...
-- Description: Canonical schema with user auth, fleet management, notifications,
--              bookings dispatch, and hardened relational integrity.
-- =============================================================================
...
-- Note: phone_number and email have automatic unique B-tree indexes from UNIQUE constraints
CREATE INDEX IF NOT EXISTS idx_dka_users_role ON dka_users(role);
CREATE INDEX IF NOT EXISTS idx_dka_users_created_at ON dka_users(created_at DESC);
...
-- =============================================================================
-- 3. DRIVERS & FLEET OWNERS (dka_owners & cr_owners synchronization)
-- Defined before dka_vehicles to satisfy foreign key dependencies
-- =============================================================================

-- Mobile App Owners Table (dka_owners)
...
-- Backward-compatibility VIEW: cr_drivers always mirrors cr_owners (if exists) or dka_owners
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_owners') THEN
        CREATE OR REPLACE VIEW cr_drivers AS SELECT * FROM public.cr_owners;
    ELSE
        CREATE OR REPLACE VIEW cr_drivers AS SELECT * FROM public.dka_owners;
    END IF;
END $$;
...
BEGIN
    ...
    -- Only sync if external table cr_owners exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_owners') THEN
...
-- =============================================================================
-- 4. VEHICLE FLEET INVENTORY (dka_vehicles)
-- =============================================================================
CREATE TABLE IF NOT EXISTS dka_vehicles (
    vehicle_id SERIAL PRIMARY KEY,
    owner_id INTEGER REFERENCES dka_owners(owner_id) ON DELETE SET NULL,
    vehicle_type_id INTEGER REFERENCES dka_vehicle_types(vehicle_type_id) ON DELETE RESTRICT,
...
BEGIN
    ...
    -- Only sync if external table cr_vehicles exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_vehicles') THEN
        IF (TG_OP = 'DELETE') THEN RETURN OLD; ELSE RETURN NEW; END IF;
    END IF;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_vehicles') THEN
...
CREATE TABLE IF NOT EXISTS dka_bookings (
    ...
    user_id INTEGER NOT NULL REFERENCES dka_users(user_id) ON DELETE RESTRICT,
    ...
    booking_status VARCHAR(50) NOT NULL DEFAULT 'Pending' CHECK (booking_status IN ('Pending', 'Confirmed', 'Completed', 'Cancelled')),
    assigned_vehicle_plate VARCHAR(50),
    assigned_vehicle_model VARCHAR(100),
    assigned_driver_id INTEGER REFERENCES dka_owners(owner_id) ON DELETE SET NULL,
    assigned_driver_name VARCHAR(120),
    assigned_driver_phone VARCHAR(30),
    final_fare VARCHAR(50),
    final_fare_npr INTEGER,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_dka_notifications_user_unread ON dka_notifications(user_id, is_read);
...
-- Initial backfill from existing cr_owners into dka_owners (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_owners') THEN
        INSERT INTO dka_owners (
        ...
        PERFORM setval(pg_get_serial_sequence('dka_owners', 'owner_id'), COALESCE((SELECT MAX(owner_id) FROM dka_owners), 1));
    END IF;
END $$;

-- Initial backfill from existing cr_vehicles into dka_vehicles (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cr_vehicles') THEN
        INSERT INTO dka_vehicles (
        ...
        PERFORM setval(pg_get_serial_sequence('dka_vehicles', 'vehicle_id'), COALESCE((SELECT MAX(vehicle_id) FROM dka_vehicles), 1));
    END IF;
END $$;

-- Canonical seed accounts with normalized E.164 phone numbers and unique emails
INSERT INTO dka_users (full_name, phone_number, email, password_hash, role, is_active, is_verified)
VALUES
    ('Samman Chhetri', '+9779851363783', 'samman@drivekendra.com', '$2b$10$demoHashedPasswordSamman1234567890', 'customer', TRUE, TRUE),
