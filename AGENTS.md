...
3. **Patches Folder**: For pending database updates, alterations, or incremental changes, create a new numbered patch file inside [`database/patches/`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/patches/) (patches `001`-`009` are consolidated into `database.sql`; new patches start at `010_your_feature.sql`).
4. **Patch Consolidation & Cleanup**: Once patches are applied by the administrator/developer to the target database and confirmed in [`database/database.sql`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/database.sql), delete the applied patch files from `database/patches/`.
5. **Execution Constraint**: **NEVER** run SQL queries directly on any live database yourself. Only produce the SQL files in `database/database.sql` and `database/patches/` for manual or administrator application.
...
- **Mobile Client (`src/`)**: Built with React Native 0.86.3, Expo SDK 57, React 19.2.3, TypeScript strict mode, React Navigation v7 (4-tab bottom navigation + stack modal screens), an isolated 2FA-secured Admin Portal (`AdminNavigator`), production UX hardening with biometric unlock cards & PIN fallback, customer self-service cancellation, and Apple Guideline 5.1.1(v) account deletion.
- **Backend API (`server/`)**: Built with Hono v4, Node.js (`tsx watch`), Zod v4 validation, and `pg` PostgreSQL connection pool.
- **Database (`database/`)**: PostgreSQL 15+ canonical schema in [`database/database.sql`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/database.sql), external partner fleet schema in [`database/cr_database.sql`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/cr_database.sql), and incremental patches in [`database/patches/`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/patches/).
...
2. **Database Changes**:
   ...
   - If modifying partner fleet structures, also update [`database/cr_database.sql`](file:///c:/Users/Lenovo/Desktop/DriveKendra/DriveKendra.Mobile/database/cr_database.sql).
...
5. **Interactive Mapping & Free Geocoding**:
   ...
   - Handle GPS permission denials gracefully by directing users to OS settings via `Linking.openSettings()`.
6. **Himalayan Resilience & Offline Handling**:
   ...
   - Protect booking submissions from rapid-tap duplicate reservations with submission state flags and unique `X-Idempotency-Key` headers.
   ...
   - Include in-screen network error recovery banners with retry actions (`MyTripsScreen.tsx`).
7. **Production UX Hardening & App Store Compliance**:
   - Support Apple App Store Guideline 5.1.1(v) account deletion via `DELETE /api/users/account` and in-app confirmation modal (`ProfileScreen.tsx`), deleting tokens/notifications and anonymizing ledger data.
   - Provide customer self-service cancellation for pending bookings via `PATCH /api/bookings/:id/cancel` with instant UI refresh (`MyTripsScreen.tsx`).
   - Gate demo credentials and administrative quick-links behind `__DEV__` in authentication screens.
   - Provide in-screen biometric unlock cards with instant fallback to PIN/OTP or password when biometric authentication is canceled or unavailable.
8. **Strict Admin Stack Isolation & Dispatch Desk Workflow**:
   ...
   - Admin dashboard API operations use `Promise.allSettled` to recover gracefully from partial network failures.
9. **Owner, Driver & Vehicle Bidirectional Synchronization**:
...
10. **Real-Time Push Notifications & Notification Center**:
    ...
    - Dispatch real-time lifecycle alerts for booking submissions, driver/vehicle assignments, trip cancellations, rejections, and completions using `recordAndPushTripNotification` (`server/src/push.ts`).
